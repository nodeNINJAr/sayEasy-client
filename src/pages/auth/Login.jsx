...
const Login = () => {
  ...
  const handleGuestLogin = () => {
    setLoading('guest')
    // 
    signInUsingEmailpass('suzume99@gmail.com','123456')
    .then(() => {
      setLoading('')
      toast.success(
        "🎉 Welcome ! You’ve successfully logged in as a guest. Let’s dive in and get started!"
      );
      navigate(from, { replace: true });
    })
    .catch((err) => {
      setLoading('')
      toast.error(
        "❌ Login failed! Please contact the developer and try again."
      );
      // console.log(err.message);
    });
  };
  ...
  return (
    <div className="py-10 md:py-20">
      ...
      <div className="w-full mx-auto max-w-md p-4 rounded-md shadow sm:p-8 bg-gray-900 dark:bg-gray-50 text-gray-100 dark:text-gray-800 py-14">
        <button onClick={handleGuestLogin} className="flex justify-center items-center gap-4 w-full bg-[#9A59C9] p-3 rounded-lg font-fira text-xl font-semibold mb-4">
        Guest Login {loading ==="guest" && <span className="loading loading-spinner loading-md"></span>} 
        </button>
        {/*  */}
        ...
        <div className="my-6 space-y-4">
          <button
            ...
            type="button"
            className="flex items-center justify-center gap-4 w-full p-4 space-x-4 border rounded-md focus:ring-2 focus:ring-offset-1 border-gray-400 dark:border-gray-600 focus:ring-violet-400 focus:dark:ring-violet-600"
          >
            <svg
            ...
            Login with Google {loading==="google" && <span className="loading loading-spinner loading-md"></span>}
          </button>
          {/*  */}
        </div>
        ...
        <form onSubmit={handleSignIn} className="space-y-8">
          ...
          <button
            type="submit"
            className="flex items-center justify-center gap-4 w-full px-8 py-3 font-semibold rounded-md bg-violet-400 dark:bg-violet-600 text-gray-900 dark:text-gray-50"
          >
            Login {loading === "login" && <span className="loading loading-spinner loading-md"></span>}
          </button>
        </form>
      </div>
    </div>
