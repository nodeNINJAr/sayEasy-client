...
const TutorDetails = () => {
  ...
  const handleBookTutors = async () => {
    ...
    if (user?.email !== tutorInfo?.email) {
      try {
        await axiosSecure.post("/tutor-booking", bookedInfo);
  ...
  return (
    <>
     <Helmet>
        <title>Tutor Details || sayEasy</title>
      </Helmet>
      {/* hero */}
      <Hero path={'tutor details'} />
      {/*  */}
      <div className="py-10 md:py-20 sm:w-11/12 mx-auto p-6 sm:p-0  dark:text-gray-200 transition duration-300">
        {/* Header Section */}
        <div className="flex flex-wrap justify-between items-center bg-white dark:bg-gray-700 p-6 rounded-lg shadow">
          <div className="flex items-center space-x-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-300 capitalize font-figtree">
                Expert{" "}
                <span className="text-red-400">{tutorInfo?.language}</span>{" "}
                language Tutor 🌟
              </p>
          ...
          <div className="text-right">
            ...
            <button
              disabled={user?.email === tutorInfo?.email}
              onClick={handleBookTutors}
              className="mt-2 px-5 py-2 rounded-lg font-figtree bg-green-300 text-gray-500 disabled:bg-gray-300 disabled:text-gray-400 disabled:cursor-not-allowed"
            >
              Book Now
            </button>
          </div>
        ...
        <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow mt-6 font-figtree">
          ...
          <p className="text-gray-600 dark:text-gray-300 mt-2">
            Hi! I'm {tutorInfo?.name}, a passionate {tutorInfo?.language}{" "}
            language tutor with 5+ years of experience helping students achieve
        ...
        <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow mt-6">
          ...
          <div className="mt-4 space-y-2">
            <p className="text-gray-600 dark:text-gray-300 flex items-center">
              <FaEnvelope className="mr-2 text-primary" /> Email : {tutorInfo?.email}
            </p>
            <p className="text-gray-600 dark:text-gray-300 flex items-center">
